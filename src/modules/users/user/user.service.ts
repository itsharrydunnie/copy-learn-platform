...
class UserService {
  ...
  public async createUser(data: createUserDTO): Promise<IUserDoc> {
    ...
    if (!creatorId) {
      ...
      const updateResult = await userRepository.updateUser(String(user._id), {
        createdBy: user._id,
      });
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if ((existResult.error || !existResult.data) && options.isNew) {
          try {
            ...
            const bulkUserCode = genUserCode(bulk.userType || UserType.USER);
            ...
            // Encrypt password
            await authService.encryptUserPassword(user, bulk.password);
            await user.save();
  ...
  public async updateUserProfile(
    ...
    if (data.avatar) {
      ...
      if (typeof data.avatar === "object" && (data.avatar as IFile).stream) {
    ...
    // Update the user
    const updateResult = await userRepository.updateUser(userId, updateData);
    if (updateResult.error) {
      result.error = true;
  ...
  async setUserType(
    ...
    // Validate user type
    if (![UserType.TALENT, UserType.USER].includes(data.userType)) {
      result.error = true;
    ...
    const roleAttachResult = await roleService.attachRole(user, data.userType);
   ...
   * @param userId - The user ID
   */
  async completeOnboarding(userId: string | Types.ObjectId): Promise<IResult> {
    let result: IResult = {
      error: false,
      message: "",
      code: 200,
    ...
    if (user.userType === UserType.TALENT) {
      ...
      if (talentResult.error || !talentResult.data) {
        result.error = true;
        result.message = "Talent profile not found. Please complete step 3.";
        result.code = 400;
        return result;
      }
    }
   ...
   * @param userId - The user ID
   */
  async getOnboardingStatus(userId: string | Types.ObjectId): Promise<IResult> {
    let result: IResult = {
      error: false,
      message: "",
      code: 200,
